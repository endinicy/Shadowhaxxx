...
def get_lag(data_q,plotgen):
    ...
    while not data_q.full():
        ...
        if not data_q.empty():
            rawdata=data_q.get()
            channel0.append(rawdata[0])
            channel1.append(rawdata[1])
            if time.time() > lagstart+3: # After 3 seconds of data collection, this deletes the first entry everytime a new one is added. 
                ...
                channel0.pop(0)         # This is great because it works independent of sample rate, but any delays at the beginning will reduce output size
                channel1.pop(1)
                if time.time()-lastlagtime > 1:
                    print "Computing... "
                    samplerate=float(len(channel0)/3)
                    print(lag_xcorr([channel0,channel1],samprate=samplerate))
                    lastlagtime = time.time()
